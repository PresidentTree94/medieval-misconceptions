    ...
    <header className={`fixed top-0 inset-x-0 z-1 ${scrolled || open ? "bg-background-light/95 backdrop-blur-md shadow-sm" : "bg-transparent"}`}>
      <div className="flex flex-col md:flex-row justify-between md:items-center">
        ...
        <nav className={`text-sm font-medium ${open ? "flex" : "hidden md:flex"} flex-col md:flex-row gap-1 px-6 py-6 md:py-0`}>
          {navs.map((n, index) => (
            <Link key={index} href={n.link} className={`px-3 py-2 rounded-md ${scrolled || open ? "text-subtext hover:text-primary hover:bg-primary/5" : "text-background-light/90 hover:text-background-light hover:bg-background-light/15"}`} onClick={() => setOpen(false)}>{n.text}</Link> 
          ))}
        </nav>
      </div>
    </header>
