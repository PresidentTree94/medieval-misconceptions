  ...
  return (
    <Link href={`/figures/${data.slug.current}`} className="bg-background-dark rounded-xl border border-secondary overflow-hidden group transition-colors hover:border-primary/25">
      <div className="relative aspect-square overflow-hidden">
        <Image src={data.image ? urlFor(data.image).url() : "https://placehold.co/256/png"} alt={data.image ? data.image.alt : ""} fill className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
